...
PPOL 5202 Data Visualization, semester project. Nora Zhan.
...
The two final analysis tables are included in `data/processed/`. To reproduce them from the original public files, knit the two notebooks in `code/` in order, from RStudio (Knit button) or from the R console:
...
docs/
  data_viability_notes.md       source-by-source viability notes
  tableau_prototypes_guide.md   step-by-step Tableau build for the three proposal prototypes
proposal/
  ...
  tableau_data/                 the three chart-specific datasets used in Tableau
